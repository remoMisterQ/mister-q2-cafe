...
export default async function AdminMenuPage() {
  await requireAdmin();
  const [items, categories, locations, modifierGroups] = await Promise.all([getMenuItems(), getCategories(), getLocations(), getModifierGroups()]);
    ...
    <main className="section-pad">
      <div className="container-page grid gap-6 lg:grid-cols-[240px_1fr]">
        ...
        <section className="grid gap-6">
          ...
          <MenuTable items={items} categories={categories} locations={locations} modifierGroups={modifierGroups} />
        </section>
      </div>
    </main>
  );
